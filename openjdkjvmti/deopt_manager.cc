...
class ScopedDeoptimizationContext : public art::ValueObject {
 public:
  ScopedDeoptimizationContext(art::Thread* self, DeoptManager* deopt)
    ...
    // TODO Due to the fact the jit can cause ClassLoad & ClassPrepare events to be triggered on
    // array-classes (see b/111394423) this can cause deadlocks if we are holding JVMTI raw
    // monitors and try to gain them in a jit thread.
    if (jit_ != nullptr) {
      jit_->Stop();
    }
