...
TEST_F(PatchoatTest, PatchoatRelocationSameAsDex2oatRelocation) {
  ...
  // Force single-threaded compilation to avoid non-determinism which may be caused by
  // multi-threaded compilation. We want the boot image created by this dex2oat run and the run
  // below to differ only in its base address.
  std::vector<std::string> dex2oat_extra_args;
  dex2oat_extra_args.push_back("-j1");
  std::string error_msg;
  ...
  // if (!CompileBootImageToDir(dex2oat_dir, reloc_base_addr, &error_msg)) {
